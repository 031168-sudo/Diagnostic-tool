...
function wavMetrics(buffer) {
  ...
  const bytesPerFrame = channels * 2, frames = Math.floor(dataSize / bytesPerFrame), duration = frames / rate;
  const stride = Math.max(1, Math.floor(rate * 0.5)); let rmsSum=0, peak=0, count=0, zc=0, prev=0, first=true, samples=[];
  for (let f=0; f<frames; f += stride) {
    const idx=dataStart+f*bytesPerFrame; if(idx+1>=buffer.length) break;
    const x=buffer.readInt16LE(idx)/32768; rmsSum+=x*x; peak=Math.max(peak,Math.abs(x)); count++;
    if(!first && ((x>=0)!=(prev>=0))) zc++; first=false; prev=x; samples.push(x);
  }
  return { channels, rate, bits, durationSec:Number(duration.toFixed(2)), sampledPoints:count, rms:Number(Math.sqrt(rmsSum/Math.max(1,count)).toFixed(5)), peak:Number(peak.toFixed(5)), zeroCrossings:zc };
}

async function transcribeAudio(file) {
  if (!file) return "";
  try {
    const r = await openai.audio.transcriptions.create({ model: transcribeModel, file: await import("node:fs").then(m => m.createReadStream(file.path)), language: "ru" });
    return r.text || "";
  } catch (e) { return `Транскрипция недоступна: ${e.message}`; }
}

const systemPrompt = `Ты — автомобильный диагност Alfa Diagnostic. Анализируй только предоставленные данные и явно отделяй факт от гипотезы. Сопоставляй audio, OBD, GPS и датчики по общей временной шкале. Не утверждай неисправность конкретной детали, если данные её не доказывают. Если для различения причин нужен простой дополнительный тест или вопрос владельцу — задай его.
...
При завершении заключение должно быть техническим и структурированным: исходная жалоба; что реально обнаружено в данных; корреляции и временные закономерности; возможные причины с уровнем уверенности; что проверить в первую очередь; какие контрольные проверки выполнить; ограничения анализа.`;
...
async function runAnalysis(s, extra = "") {
  ...
  const session=await readTextIfExists(dir,"session.json",500_000);
  const obd=await readTextIfExists(dir,"obd.csv",2_500_000);
  const gps=await readTextIfExists(dir,"gps.csv",1_500_000);
  const sensors=await readTextIfExists(dir,"sensors.csv",2_500_000);
  const audioFile=s.files.find(f=>f.name==="audio.wav");
  let audioMetrics={}; let transcript="";
  if(audioFile){ const b=await fs.readFile(audioFile.path); audioMetrics=wavMetrics(b); }
  s.stage="Анализ звука"; s.message="Выделяю акустические характеристики и проверяю наличие речи…"; await saveState(s);
...
app.post("/v1/diagnostics", upload.array("files",10), async (req,res) => {
  try {
    ...
    await saveState(s); res.json({id});
    runAnalysis(s).catch(async e=>{ s.state="error"; s.stage="Ошибка"; s.message=e.message; await saveState(s); });
...
app.post("/v1/diagnostics/:id/messages", async (req,res)=>{
  try { const s=await loadState(req.params.id); if(s.state!=="question") return res.status(409).json({error:"Сейчас ответ не требуется"}); const text=String(req.body?.text||"").trim(); if(!text) return res.status(400).json({error:"Пустой ответ"});
    s.state="processing"; s.stage="Уточнение диагноза"; s.message="Учитываю ответ и повторно проверяю данные…"; await saveState(s); res.json({ok:true});
    runAnalysis(s,text).catch(async e=>{s.state="error";s.stage="Ошибка";s.message=e.message;await saveState(s);});
...
app.listen(port,()=>console.log(`Alfa Diagnostic server listening on ${port}`));
