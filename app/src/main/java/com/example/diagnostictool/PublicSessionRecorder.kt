...
class PublicSessionRecorder(private val activity: NewMainActivity, private val car: Car, private val status: (String) -> Unit) : SensorEventListener, LocationListener {
    ...
    fun start() {
        ...
        try {
            obdUri = createPublicFile("obd.csv", "text/csv"); obdWriter = writerFor(obdUri!!); obdWriter!!.write("relative_ms,monotonic_ns,rpm,speed_kmh,load_pct,throttle_pct,map_kpa,coolant_c,intake_c,voltage_v\n"); obdWriter!!.flush()
            sensorsUri = createPublicFile("sensors.csv", "text/csv"); sensorsWriter = writerFor(sensorsUri!!); sensorsWriter!!.write("relative_ms,monotonic_ns,type,x,y,z\n"); sensorsWriter!!.flush()
            gpsUri = createPublicFile("gps.csv", "text/csv"); gpsWriter = writerFor(gpsUri!!); gpsWriter!!.write("relative_ms,monotonic_ns,speed_kmh,latitude,longitude,accuracy_m\n"); gpsWriter!!.flush()
    ...
    @Synchronized fun onObd(v: ObdValues, monotonicNs: Long) {
        ...
        obdWriter?.write("%.3f,%d,%s,%s,%s,%s,%s,%s,%s,%s\n".format(Locale.US, relativeMs, monotonicNs, v.rpm?.toString() ?: "", v.speed?.toString() ?: "", v.load?.toString() ?: "", v.throttle?.toString() ?: "", v.map?.toString() ?: "", v.coolant?.toString() ?: "", v.intake?.toString() ?: "", v.voltage?.toString() ?: "")); obdWriter?.flush(); obdActive = true
    }

    override fun onLocationChanged(location: Location) { if (!running.get()) return; val speed = if (location.hasSpeed()) (location.speed * 3.6).toDouble() else null; if (speed != null) lastGpsSpeedKmh = speed; val t = SystemClock.elapsedRealtimeNanos(); val relativeMs = (t - sessionStartNs) / 1_000_000.0; synchronized(this) { gpsWriter?.write("%.3f,%d,%s,%s,%s,%s\n".format(Locale.US, relativeMs, t, speed?.toString() ?: "", location.latitude, location.longitude, location.accuracy)); gpsWriter?.flush() } }
    ...
    fun stop() {
        ...
        sensorManager?.unregisterListener(this); stopLocation(); try { audioThread?.join(2000) } catch (_: Exception) {}; try { audio?.stop() } catch (_: Exception) {}; audio?.release(); audio = null
        synchronized(this) { obdWriter?.close(); obdWriter = null; sensorsWriter?.close(); sensorsWriter = null; gpsWriter?.close(); gpsWriter = null }
        try { obdUri?.let { publish(it) } } catch (_: Exception) {}; try { sensorsUri?.let { publish(it) } } catch (_: Exception) {}; try { gpsUri?.let { publish(it) } } catch (_: Exception) {}
