...
class PublicSessionRecorder(
    ...
    fun start() {
        ...
        try {
            ...
            status("Микрофон недоступен: ${e.message}")
            return
        }
    ...
    override fun onSensorChanged(event: SensorEvent) {
        ...
        synchronized(this) {
            sensorsWriter?.write("%.3f,%d,%s,%.7f,%.7f,%.7f\n".format(Locale.US, relativeMs, t, type, event.values[0], event.values[1], event.values[2]))
            sensorsWriter?.flush()
        }
    }
    ...
    private fun writeMeta(sampleRate: Int) {
        File(pcmFile!!.parentFile, "session.json").writeText(
            """{
...
"""
    ...
    private fun writeWavHeader(out: OutputStream, rate: Int, channels: Int, bits: Int, dataSize: Long) {
        ...
        h.put("data".toByteArray()); h.putInt(dataSize.toInt()); out.write(h.array())
    }

    private fun writerFor(uri: android.net.Uri): BufferedWriter =
        (activity.contentResolver.openOutputStream(uri) ?: error("Не удалось открыть $uri")).bufferedWriter()

    private fun createPublicFile(name: String, mime: String): android.net.Uri {
        if (Build.VERSION.SDK_INT < 29) {
            val dir = File(Environment.getExternalStoragePublicDirectory(Environment.DIRECTORY_DOWNLOADS), "DiagnosticTool/sessions/$sessionName")
            dir.mkdirs()
        ...
        val values = ContentValues().apply {
            ...
            put(MediaStore.Downloads.RELATIVE_PATH, "Download/DiagnosticTool/sessions/$sessionName")
