...
class TargetElm327Ble(
    ...
    private fun startElmSession() {
        thread(name = "elm327-session") {
            ...
            running = true; commandIndex = 0; listener.onState("ELM327 ${selectedDevice?.address ?: ""} готов; OBD опрашивается"); pollLoop()
        }
    }
    ...
    private fun pollLoop() {
        while (running) {
            val command = commands[commandIndex++ % commands.size]; val response = sendAndWait(command, 2000)
