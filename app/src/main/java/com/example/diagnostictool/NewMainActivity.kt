...
class NewMainActivity : AppCompatActivity() {
    ...
    private fun showObdDevices(devices: List<TargetElm327Ble.DeviceInfo>) {
        if (devices.isEmpty()) {
            AlertDialog.Builder(this).setTitle("OBD-адаптеры не найдены").setMessage("Проверьте питание ELM327 и Bluetooth.").setPositiveButton("OK", null).show()
            return
        }
        ...
        AlertDialog.Builder(this).setTitle("Выберите OBD-адаптер")
            .setSingleChoiceItems(labels, -1) { dialog, which ->
                ...
                dialog.dismiss()
                status.text = "Подключение: ${selected.label().replace("\n", " — ")}"
    ...
    private fun showCarsDialog() {
        ...
        AlertDialog.Builder(this)
            ...
            .setSingleChoiceItems(labels, selected) { dialog, which ->
                carStore.select(cars[which].id)
                refreshCarUi()
                dialog.dismiss()
            }
            .setNeutralButton("Изменить") { _, _ -> showCarEditor(cars[selected], false) }
            .setPositiveButton("Добавить") { _, _ -> addCarAndReturnToDiagnostic() }
    ...
    private fun showCarEditor(car: Car, returnToDiagnostic: Boolean) {
        ...
        val plate = field("Госномер (необязательно)", car.plate)
        val notes = field("Дополнительная информация", car.notes)
        val scroll = android.widget.ScrollView(this).apply { addView(form) }
        AlertDialog.Builder(this)
            .setTitle(if (car.make.isBlank() && car.model.isBlank()) "Добавить автомобиль" else "Автомобиль")
            .setView(scroll)
            .setNegativeButton("Отмена") { _, _ -> if (returnToDiagnostic && carStore.all().isEmpty()) showFirstCarDialog() }
            .setPositiveButton("Сохранить") { _, _ ->
                ...
                car.drive = drive.text.toString().trim(); car.mileage = mileage.text.toString().trim(); car.vin = vin.text.toString().trim()
                car.plate = plate.text.toString().trim(); car.notes = notes.text.toString().trim()
    ...
    override fun onRequestPermissionsResult(request: Int, permissions: Array<out String>, results: IntArray) {
        ...
        if (request == requestCode && pendingConnect && results.all { it == PackageManager.PERMISSION_GRANTED }) { configureObd(); obd.scan() }
    ...
    private fun toggleRecording() {
        ...
        if (recorder == null) {
            ...
            val r = recorder ?: return; r.stop(); recorder = null; recordButton.text = "НАЧАТЬ ЗАПИСЬ"; showComplaintDialog(car, r.sessionName)
        }
    }

    private fun showComplaintDialog(car: Car, sessionName: String) {
        val input = EditText(this).apply { hint = "Опишите неисправность (необязательно)"; minLines = 5; gravity = android.view.Gravity.TOP }
        AlertDialog.Builder(this).setTitle("Описание неисправности").setView(input)
            ...
            .setPositiveButton("Сохранить") { _, _ -> recordStore.add(car.id, sessionName, input.text.toString().trim()) }.show()
    }
