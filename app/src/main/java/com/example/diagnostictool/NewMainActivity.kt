...
class NewMainActivity : AppCompatActivity() {
    ...
    private fun configureObd() {
        ...
        status.text = "OBD-адаптер: не выбран"
    }
    ...
    private fun showCarsDialog() {
        val cars=carStore.all(); if(cars.isEmpty()){showCarEditor(carStore.create());return}
        ...
        AlertDialog.Builder(this).setTitle("Автомобили").setSingleChoiceItems(labels,selected){dialog,which->carStore.select(cars[which].id);refreshCarUi();dialog.dismiss()}
            .setNeutralButton("Изменить"){_,_->showCarEditor(cars[selected])}.setNegativeButton("Закрыть",null).setPositiveButton("Добавить"){_,_->showCarEditor(carStore.create())}.show()
    }

    private fun showCarEditor(car: Car) {
        val form=LinearLayout(this).apply{orientation=LinearLayout.VERTICAL;setPadding(40,8,40,0)}
        fun field(hint:String,value:String="")=EditText(this).apply{this.hint=hint;setText(value);setSingleLine(false);form.addView(this)}
        ...
        AlertDialog.Builder(this).setTitle(if(car.make.isBlank()&&car.model.isBlank())"Добавить автомобиль" else "Автомобиль").setView(scroll).setNegativeButton("Отмена",null).setNeutralButton("Удалить"){_,_->if(carStore.all().any{it.id==car.id}){carStore.delete(car.id);refreshCarUi()}}
